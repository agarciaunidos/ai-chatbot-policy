...
import boto3
import json
from aws_secrets_initialization import PINECONE_API_KEY
...
def get_similar_prompts(query, k=5):
    ...
    #return [(match['metadata']['text'], match['metadata']['user'], match['score']) for match in results['matches']]
    return [(f"{match['metadata']['text']} {match['metadata']['user']}", match['score']) 
